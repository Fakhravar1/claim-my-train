create table if not exists public.api_call_events (
  id uuid primary key default gen_random_uuid(),
  requested_at timestamptz not null default now(),
  source text not null check (source in ('user', 'scheduled', 'collector')),
...
drop policy if exists "Anyone can view api call events" on public.api_call_events;
create policy "Anyone can view api call events"
on public.api_call_events
for select
using (true);

create index if not exists idx_api_call_events_requested_at on public.api_call_events(requested_at desc);
create index if not exists idx_api_call_events_source on public.api_call_events(source);
create index if not exists idx_api_call_events_route on public.api_call_events(origin_stop_id, destination_stop_id);
...
source_breakdown as (
  select
    e.source,
    count(*)::int as calls
  from public.api_call_events e
  where e.requested_at >= since_ts
  group by e.source
),
top_routes as (
  select
    coalesce(e.origin_stop_name, e.origin_stop_id, 'Unknown') as origin,
    coalesce(e.destination_stop_name, e.destination_stop_id, 'Unknown') as destination,
...
select jsonb_build_object(
  ...
  'source_breakdown', coalesce((
    select jsonb_agg(
      jsonb_build_object('source', s.source, 'calls', s.calls)
      order by s.source
    )
    from source_breakdown s
  ), '[]'::jsonb),
  'top_user_routes', coalesce((
    select jsonb_agg(
      jsonb_build_object('origin', r.origin, 'destination', r.destination, 'searches', r.searches)
      order by r.searches desc, r.origin, r.destination
    )
    from top_routes r
  ), '[]'::jsonb)
);
