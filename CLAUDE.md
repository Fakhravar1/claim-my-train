...
- ✅ **Claim tracking.** "My claims" tab in `src/pages/Settings.tsx` lists each claim (route, travel date, delay tier, status badge: pending / form ready / submitted / error) via `useMyClaims`. A **Download PDF** button appears once `status='generated'`; it calls `supabase.storage.from('claims').createSignedUrl(...)`. That download needs a Storage RLS policy — **"Users can read own claim PDFs"** on `storage.objects`: `select` to `authenticated` where `bucket_id='claims' and (storage.foldername(name))[1] = auth.uid()::text` (matches the worker's `{user_id}/{claim_id}.pdf` layout). The worker itself writes with the service-role key and bypasses this.
