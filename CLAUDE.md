...
- **Claim filing pipeline (operator-routed since 2026-06-23):** Python worker in `claim-worker/` polls `public.claims` for `status in ('pending','sj_authorized')` and dispatches on `claim.purchasing_operator` (`worker.py` `HANDLERS`):
  ...
  - Runs on GitHub Actions (`.github/workflows/claim-pdf-worker.yml`, daily `0 6 * * *` + `workflow_dispatch`; now also `playwright install chromium`), using `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` repo secrets (service-role key bypasses RLS — backend-only). Render cron is paid, so this rides on Actions alongside dbt.
