...
**Storage per traffic added (measured 2026-06-11, one 2h batch at 8 stations = 559 events):** TV raw ≈ **1.9 kB/event** (the `raw` jsonb dominates — the §15 column-slimming lever at scale), `int_stop_events` ≈ 330 B/event. At 8 stations ≈ ~6.7k events/day → ~13 MB/day raw TV, unbounded (no prune yet). Naive 1000-station projection ≈ ~1.6 GB/day raw — the missing `raw_train_announcements` prune (gap #1 above) becomes mandatory well before that, and `int_stop_events` (~250 MB/day at that scale, also currently unpruned) needs its ~70 d retention job too. REST Danish whitelist (`int_stop_events` `rest` CTE): `25314` CPH Airport (Kastrup), `23657` Tårnby, `25313` Ørestad, `25315` København H. **Østerport is not yet in REST data** (trains continue there — `line_terminus` says "København Østerport" — but our collector doesn't poll that stop's board) — add its `stop__id` to the whitelist once it appears. Widen TV by adding LocationSignature codes (look up in `ref_stations.tv_signature`) and redeploying; widen Danish by adding ids to the whitelist.
