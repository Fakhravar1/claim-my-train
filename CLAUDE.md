...
**Frontend layout (redesigned 2026-06-17 — "Daylight"):** `/` now serves the **merged single-page app** `src/pages/DaylightApp.tsx` (design handoff option G, "Daylight"): one scroll page — nav → hero → **live network board** → value props ("Så funkar det") → footer — plus the multi-step **claim modal** (`ClaimModal`) and the near-threshold **eligibility modal** (`EligibilityModal`). It is **public** and is the primary surface, replacing both the old marketing landing AND the separate region "board" pages; the `<ProtectedFromAuth>` bounce is **gone**, so signed-in users stay on `/`. The nav is **auth-aware** (signed in → account menu: Inställningar / Mina förseningar / Logga ut; signed out → "Logga in" opens the in-modal sign-in pop-up). **Login is an in-modal pop-up** (preferred over a separate page — reverted 2026-06-17): the signed-out nav "Logga in", the "Ställ in pendlarvanor" value-prop (signed-out), and the in-claim "Nästa" while signed out all open the claim modal's **account view** (`AccountIntent` `login`/`save`) with **Fortsätt med Google** (`signInWithGoogle`, the real OAuth) and **Fortsätt med e-post** (links to `/login` for the email/password path). The account view's `submitted` intent is the post-claim success screen. A signed-in "Ställ in pendlarvanor" goes straight to `/settings`. Theme: `src/themes/daylight/daylight.css` (ported near-verbatim from the design prototype, **scoped under `.cmt-daylight` / `.cmt-daylight-scrim`** so the generic class names — `.btn`, `.row`, `.modal`, `.tag` — can't leak into shadcn), injected via `src/hooks/useDaylightStyles.ts`; fonts Schibsted Grotesk + IBM Plex Mono loaded in `index.html`. Components in `src/components/daylight/` (`shell.tsx` = Nav/Hero/ValueProps/Footer, `Board.tsx`, `ClaimModal.tsx`, `EligibilityModal.tsx`, `primitives.tsx`, `icons.tsx`). **The region routes still exist** — `/regions/skanetrafiken`, `/regions/skanetrafiken/delay-alerts`, `/regions/skanetrafiken/claim-review` — kept because the **digest emails deep-link to them** (§16); they keep their older cmt-* look (`useAppShellStyles` + `regional-app-base.css`). `/login` and `/settings` stay on the shadcn theme (Daylight retheme deferred). **Orphaned but not deleted:** `src/pages/Landing.tsx` + `src/components/landing/*` + `ProtectedFromAuth` are no longer routed (tree-shaken out of the build) — retire when convenient. SL/Västtrafik only ever appeared as inert "Coming soon" cards on that now-unrouted landing. DaylightApp and the region routes are lazy-loaded.
...
- **Frontend:** `src/` (React/TypeScript + Vite + Tailwind + shadcn/ui). **Primary surface is the merged "Daylight" single-page app at `/`** (`src/pages/DaylightApp.tsx`; theme `src/themes/daylight/daylight.css` via `src/hooks/useDaylightStyles.ts`; components in `src/components/daylight/` — see §1 and §7). The region routes still serve the older cmt-* look: Skåne departures at `/regions/skanetrafiken` (`SkanetrafikenApp.tsx`) and claimable delays at `/regions/skanetrafiken/delay-alerts` (`SkanetrafikenDelayAlerts.tsx`) — both public, styled via `src/hooks/useAppShellStyles.ts` + `src/themes/regional-app-base.css`, kept mainly for digest deep-links. Shared region UI in `src/components/region/` (`SkaneBand.tsx`, `RegionDepartureCard.tsx`, `RegionUserMenu.tsx`, `StationCombobox.tsx`). The old `Landing.tsx` (+ `src/components/landing/*`, `useLandingStyles.ts`, `landing-base.css`) is now unrouted/orphaned. In-app shadcn theme in `src/index.css` is retuned to the Skåne palette so `/login` / `/settings` feel continuous (Daylight retheme of those two deferred).
...
**Pages:**
- `src/pages/DaylightApp.tsx` — **the merged "Daylight" single-page app at `/`** (hero + live board + claim/eligibility modals); the primary surface (§1).
- `src/pages/regions/SkanetrafikenApp.tsx` — Skåne departures cards page at `/regions/skanetrafiken` (older cmt-* look; kept for digest deep-links)
- `src/pages/regions/SkanetrafikenDelayAlerts.tsx` — claimable delays at `/regions/skanetrafiken/delay-alerts`
- `src/pages/regions/SkanetrafikenClaimReview.tsx` — bulk claim review at `/regions/skanetrafiken/claim-review` (digest links here)
- `src/pages/Settings.tsx`, `src/pages/Login.tsx`, `src/pages/NotFound.tsx`
- `src/pages/Landing.tsx` — **orphaned** old marketing landing (no longer routed; retire when convenient)

**Hooks:** `src/hooks/useStations.ts`, `src/hooks/useJourneys.ts`, `src/hooks/useNetworkBoard.ts` (Daylight board's date-only network sample), `src/hooks/useDaylightStyles.ts` (`/` theme inject), `src/hooks/useAppShellStyles.ts` (region pages), `src/hooks/useLandingStyles.ts` (orphaned landing). Status tiers: `src/lib/daylightStatus.ts`.
...
- `src/themes/daylight/daylight.css` — **the "Daylight" theme for `/`** (teal `#0E8C7E` accent, Schibsted Grotesk + IBM Plex Mono, bright white board, soft radii). Ported near-verbatim from the design prototype; **every rule scoped under `.cmt-daylight` (page) or `.cmt-daylight-scrim` (modal portal)** so generic class names don't leak. Injected via `useDaylightStyles()`; removed on unmount so `/login` and `/settings` keep shadcn. Fonts loaded in `index.html`. When editing, keep the scoping prefix on every selector.
- `src/themes/landing-base.css` — base CSS tokens (cmt-* palette, Fraunces/Inter type) used by the (now orphaned) landing.
...
- **OPEN as of 2026-06-17: Supabase Auth Site URL / Redirect URLs likely still point at the old domain/localhost, breaking Google sign-in.** After moving prod to `qvitta.nu`, Google OAuth was observed redirecting to `http://localhost:8080/#access_token=...` instead of back to the app — Supabase falls back to its configured **Site URL** whenever the requested `redirectTo` (built from `window.location.origin` in `AuthContext.signInWithGoogle`, `src/contexts/AuthContext.tsx:119`) isn't in the **Redirect URLs** allow-list. Fix is dashboard-only (Authentication → URL Configuration): set Site URL to `https://qvitta.nu` and add `https://qvitta.nu/**` to the allow-list (keep `http://localhost:8080/**` for local dev if still used). Not yet confirmed fixed — verify next time someone signs in.
