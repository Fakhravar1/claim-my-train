...
**Current coverage (SKÅNE + SJ TRUNK + SL PENDELTÅG + VGR RAIL, collector v15).** `STATIONS` in `collect-train-announcements` lists **all ~80 Skåne stations** (crosswalked, verified 2026-06-16), **the SJ trunk triangle Stockholm–Malmö–Göteborg** (21 signatures added 2026-06-23: `Cst Flb Söö` Stockholm hub; `Nr Lp My Tns N Av` Södra stambanan; `K Hpbg Lå T Sk F Hrc A G` Västra stambanan; `Mdn Kb Vbc Fabp` Västkustbanan — all `name_match=true`), **the SL pendeltåg network** (added 2026-06-24), and **the full Västra Götaland rail network in-box** (55 signatures added 2026-06-25, collector v15 — Göteborg commuter lines Alingsås/Kungsbacka/Älvängen/Borås plus the outer ring out to Trollhättan, Borås C, Vara, Kinna/Skene, Lysekil). **~207 stations total.** Göteborg-area trains carry `information_owner='Västtrafik'` (verified at `G`: 1,315 announcements) → the cards label them **"Västtrafik"** automatically via the existing operator pass-through (`int_stop_events` maps `information_owner → operator`; no frontend change). **VGR crosswalk caveat:** `Gas` (Gamlestaden), `Krd` (Kållered), `Bhs` (Bohus), `Än` (Älvängen) resolved to a nearest-coord REST stop ~150–300 m off (the §15 Ramlösa class) — display stays correct (TV's `AdvertisedLocationName` is canonical), only `route_distance_km` is slightly soft; fix later via a ResRobot `location.name` lookup if a 150 km band-edge case needs it. **Stockholm C (`Cst`) was reclassified from monitoring-only to a CLAIMABLE corridor** — removed from the `int_stop_events` TV-exclusion list (now EMPTY). Verified 2026-06-23: Malmö C↔Stockholm C journeys pair with operator `SJ` and `route_distance_km` ≈ 615 km (≥150 → EU band). Long-haul pairings fill in over a few hourly cron cycles (both endpoints must land in one ~2 h window). The pull `limit` is 20000 (a full pull at 207 stations is ~5.3k events; §15 gap #2's true pagination still only matters at the ~1000-station target). **Önnestad (`Önd`) remains polled-but-invisible** (null `ref_stations.rest_area_id` → dropped by the 740-regex join; resolve via ResRobot `location.name`). To add a station: look up `tv_signature` in `ref_stations`, append to `STATIONS`, redeploy (**keep `verify_jwt=false`**); add to the `int_stop_events` TV-exclusion list only if it should be monitoring-only.
