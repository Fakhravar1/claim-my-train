...
- **Claim filing pipeline (operator-routed since 2026-06-23):** Python worker in `claim-worker/` polls `public.claims` for `status in ('pending','sj_authorized','hlt_authorized','kalmar_authorized','vy_authorized')` and dispatches on `claim.purchasing_operator` (`worker.py` `HANDLERS`):
  ...
  - Runs on GitHub Actions (`.github/workflows/claim-pdf-worker.yml`, **every 15 min `*/15 * * * *`** since 2026-06-30 — bumped from daily so a filed claim submits within ~one cron cycle; GH free-tier jitter makes it ~15 min–1 h in practice — plus `workflow_dispatch`; also `playwright install chromium`), using `SUPABASE_URL` + `SUPABASE_SERVICE_ROLE_KEY` repo secrets (service-role key bypasses RLS — backend-only). Render cron is paid, so this rides on Actions alongside dbt. NB the daily→15-min bump raises Actions-minute burn (Chromium install per run); if minutes get tight, cache the Playwright browser or gate the install on pending claims.
...
- **SEO.** The app is a client-rendered SPA with minimal crawlable content. `public/sitemap.xml` + `public/llms.txt` exist and were trimmed to the live routes 2026-06-18 (`/`, `/settings`; `/claim-review` is intentionally `noindex` — it's a per-user digest landing). Still need per-route meta/title tags (react-helmet is wired — `DaylightApp`/`ClaimReview` set titles), richer OpenGraph/Twitter cards, and semantic headings. Consider SSR/prerender for `/` (`src/pages/DaylightApp.tsx`) since it's now the sole acquisition surface.
