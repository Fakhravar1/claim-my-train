...
**Current coverage.** TV (`STATIONS` in `collect-train-announcements`, v7): `Mc` Malmö C, `Tri` Triangeln, `Hie` Hyllie, `Lu` Lund C, `Kg` Kävlinge, `Lkö` Landskrona, `Hb` Helsingborg C, `Hbgb` Ramlösa (+ `Ram`, polled but emits nothing — see below).
...
**Storage per traffic added (measured 2026-06-11, one 2h batch at 8 stations = 559 events):** TV raw ≈ **1.9 kB/event** (the `raw` jsonb dominates — the §15 column-slimming lever at scale), `int_stop_events` ≈ 330 B/event. At 8 stations ≈ ~6.7k events/day → ~13 MB/day raw TV, unbounded (no prune yet). Naive 1000-station projection ≈ ~1.6 GB/day raw — the missing `raw_train_announcements` prune (gap #1 above) becomes mandatory well before that, and `int_stop_events` (~250 MB/day at that scale, also currently unpruned) needs its ~70 d retention job too. REST Danish whitelist (`int_stop_events` `rest` CTE): `25314` CPH Airport (Kastrup), `23657` Tårnby, `25313` Ørestad, `25315` København H, `25318` Nørreport, `25317` Østerport (corridor terminus — added 2026-06-12 with their `860…` extIds `860000646`/`860000650` in `collect-raw-departures` v17; both repo + deployed copies updated). Widen TV by adding LocationSignature codes (look up in `ref_stations.tv_signature`) and redeploying; widen Danish in TWO places — the collector's `CORRIDOR_STOPS` (`860…` extId) AND this whitelist (the short `stop__id`, read from `raw_departures` after the first poll).

**Known scaling gaps before the ~1000-station expansion** (ingestion side — the dbt chain is ready):
1. **`raw_train_announcements` has NO prune job** — TV raw upserts in place (one row per activity, linear growth in events, not polls), but at 1000 stations that's still ~10⁵–10⁶ rows/day accumulating forever. Needs a pg_cron prune like `prune-raw-departures-10d` before scale-up; retention must stay ≥ the int incremental lookback (6 h) with margin.
...
## 16. Claim digest emails (built 2026-06-12)

Opt-in email listing late departures on the user's saved commute, with one-click bulk filing.

**Pipeline:** pg_cron (`send-claim-digest-daily` `0 18 * * *` UTC ≈ 20:00 Stockholm; `send-claim-digest-weekly` `0 18 * * 0`) → `send-claim-digest` edge function (**deploy-only, no repo copy; `verify_jwt=false`** — the §15 cron trap) → Resend API (`RESEND_API_KEY` secret). Selection per user: `public.v_claimable_journeys` rows matching `(commuter_from→commuter_to)` within the outbound window ∪ reverse within the return window (Stockholm local time of `origin_scheduled`; **an unset window = that direction matches all day**), minus already-claimed (`claims` by journey_key), minus already-digested (`digest_log`). **No new journeys → no email.** The email lists the new ones + a standing-count line, and links to the review page with ALL unclaimed keys. After a successful send, journeys are logged to `public.digest_log` (unique `(user_id, journey_key)` — a journey is digested once per user, ever). Test with body `{"frequency":"daily","dryRun":true}` (returns would-send per user, sends/logs nothing). Verified 2026-06-12: real send of 8 journeys, second run sends 0 (idempotent).

**Settings:** `profiles.digest_frequency` (`off`(default)/`daily`/`weekly`, CHECK-constrained; migration `20260612120000`) — control lives under Settings → Commuter habits, next to the commute stations/windows it depends on. **Remember the §3 AuthContext footgun applied here:** `digest_frequency` was added to the `fetchProfile` explicit column list.
