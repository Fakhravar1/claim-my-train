...
- **dbt orchestration:** GitHub Actions workflow (`.github/workflows/dbt-run.yml`) runs `dbt build` on a `*/15` schedule plus `workflow_dispatch`. Secrets `SUPABASE_DB_{HOST,PORT,USER,PASSWORD,NAME}` hold the **session-pooler** connection string (port 5432) — not the transaction pooler (port 6543), which breaks dbt's multi-statement transactions and DDL. Reality check: GitHub's free-tier scheduled actions are best-effort with significant jitter — typical observed cadence is 1–4 hours between runs, not 15 min. Acceptable for the 60-day reklamation horizon; if sub-hour freshness ever matters, switch to Render Cron / Modal / a self-hosted runner. **Cron triggering (changed 2026-07-01): scheduled jobs are now fired by an external HTTP cron service, [cron-jobs.org](https://cron-jobs.org), hitting the endpoints every 15 min — replacing the in-DB pg_cron / GitHub-Actions-`schedule` pacing as the trigger. This gives reliable 15-min cadence and removes the GitHub free-tier scheduling jitter (the "1–4 h between runs" caveat above no longer applies to jobs driven this way). The `workflow_dispatch` / edge-function endpoints and `verify_jwt=false` still matter (cron-jobs.org POSTs headerless, same trap as pg_cron — §15).**
...
- **Claim filing pipeline (operator-routed since 2026-06-23):** Python worker in `claim-worker/` polls `public.claims` for `status in ('pending','sj_authorized','hlt_authorized','kalmar_authorized','vy_authorized')` and dispatches on `claim.purchasing_operator` (`worker.py` `HANDLERS`):
  - **`skanetrafiken` / NULL** → the original path (`fill_template.py`, reportlab + pypdf + pillow): fill the Skånetrafiken reklamation PDF from the claim's journey snapshot + the user's `profiles` row, **stamp the signature PNG above the Underskrift line**, upload to the private `claims` Storage bucket, flip to `generated` (or `error`). Generate-and-store only; no auto-submission.
