...
### REST + Trafikverket conforming layer (built 2026-06-10, branch `feat/stg-train-announcements`)

Two dbt models merge the TV feed with REST. Both are views; both **pushed but NOT on `main`** (so the mirror Action hasn't shipped them to prod):
- `models/staging/stg_train_announcements.sql` — clean/cast/normalise `raw_train_announcements` onto the conformed stop-event vocabulary `stg_departures` emits: `ActivityType` Ankomst/Avgang → `event_type` arrival/departure, `realtime = coalesce(actual, estimated)`, `delay_seconds = realtime − scheduled` (signed, NULL when no realtime signal — **not** coerced to 0), `is_realized`. **No joins** (staging convention). Named `delay_seconds` from the start, fixing the §13 #7 `arrival_delay` misnomer.
- `models/intermediate/int_stop_events.sql` — the **conforming layer**: unions REST + TV onto one grain `(train_number, station_id, event_type, service_date)` with precedence **TV outranks REST**, resolved by a single ranking window that *also* does the §5 intra-source latest-poll dedup. `ref_stations` is registered as a new `reference` source in `sources.yml` (it's edge-function-built, **not** dbt-managed, so it can't be `ref()`'d — `source()` it).

**Crosswalk fact, verified — the load-bearing detail.** REST `stop__id` is the **SHORT** id (`3`, `1586`, `1587`), NOT the `740…` form — REST strips the `740` prefix, and Danish stops reuse the short namespace (København H = `25315`). So **conform on the short id**; TV maps in via the identity `right(ref_stations.rest_area_id, 6)::int = stop__id`, gated to `^740[0-9]{6}$` (Swedish ids only — the gate also excludes the ~33 foreign resolved rows whose last-6 could collide). Train match: REST `trip__technical_number` (**integer**) ↔ TV `advertised_train_ident` (**text**) — conform to text. Verified live: 0 grain violations across ~58.8k conformed events; train 1061 @ Malmö C correctly resolves to the TV measurement over the REST duplicate.

**`fct_departures` is deliberately NOT wired to TV yet.** A source-swap is **impossible**: `fct_passenger_journeys` self-joins `fct_departures` on `trip__trip_id` + `stop_sequence`, and TV provides **neither** (no GTFS trip id, no stop ordering). The only correct wire is a **delay-overlay** — keep `fct_departures` REST-structured, left-join the TV measurement (`int_stop_events` where `source='tv'`) on `(technical_number, stop__id::int, event_type, local_date)`, and `coalesce(tv.delay_seconds, arrival_delay)` / `coalesce(tv.canceled, canceled)`, plus a `delay_source` column for audit. Requires carrying `trip__technical_number` into `fct_departures` (currently dropped). This **changes `is_claimable` outcomes**, so it's §8 high-stakes — apply only after explicit review, never autonomously.

**Overlay impact, measured at Malmö C** (only TV-covered stop currently): 278 overlapping arrivals, **193 (69%) differ** in delay value, **2 cross the 20-min claimable threshold (claimable flips)**, max divergence **2729 s (~45 min)**. This is *why* TV-priority matters and why the wire is gated on review.

**Incremental coupling (open before shipping the overlay).** A late **TV-only** delay revision (after REST has gone quiet for that trip) can be missed, because the incremental trip-selection keys on REST's `ingested_at`. Lean = §13 **Plan B** analog: accept the rare miss + periodic full-refresh reconcile; carry `ingested_at = greatest(rest, tv)` so the watermark stays honest. Both feeds re-poll recent trips within the lookback, so misses are edge cases.

**Widening TV coverage = editing one array.** TV identifies stations by `LocationSignature` (short codes: `Mc`=Malmö C, `Lu`=Lund C, `Hb`=Helsingborg C, `G`=Göteborg C, `Cst`=Stockholm C). The collector hardcodes `const STATIONS = ['Mc']`; add codes to that array and redeploy `collect-train-announcements`. Each becomes another OR'd `<EQ name="LocationSignature">` in the Trafikverket query. No wildcard exists. Look codes up in `ref_stations.tv_signature` by name. The conforming layer already handles every station the moment it's added.
