...
**`fct_departures` / `fct_passenger_journeys` are NOT touched.** The earlier precedence-window + fct delay-overlay design was **dropped as overkill** — it solved feed *overlap* that doesn't exist once REST is scoped to Denmark. Don't reintroduce it.

**Journey mart (built): `models/marts/fct_stitched_journeys.sql`** (view). Pairs `int_stop_events` — an origin DEPARTURE to a later ARRIVAL of the **same train number within a 12 h window** (NOT `service_date` equality: a window handles §6 cross-midnight and the < 24 h recurrence keeps each physical run separate). Ordered by **scheduled time**, not `stop_sequence` (TV has none). Excludes `origin.station_id = dest.station_id` (drops self-loops from trains that revisit a stop). Grain `(train_number, service_date, origin_stop_id, destination_stop_id)`, tested. Destination delay → `is_claimable` (v1 rule: ≥20 min or cancelled). Verified: MVP corridor produces Malmö C↔København H both directions; train 1053 (2026-06-10) Malmö C→København H +20.7 min correctly flags claimable. Separate from the REST-only `fct_passenger_journeys` path.

**Widening coverage:** TV stations via the `STATIONS = ['Mc']` array in `collect-train-announcements` (LocationSignature codes — `Mc`=Malmö C, `Lu`=Lund C, `Hb`=Helsingborg C, …; look up in `ref_stations.tv_signature`, redeploy after editing). Danish stops via the whitelist in `int_stop_events`' `rest` CTE (`stop__id in ('25315')`).
