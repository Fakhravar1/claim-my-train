...
raw_train_announcements + raw_departures(Danish stops) + ref_stations
  → stg_train_announcements / stg_departures → int_stop_events (disjoint union)
  → fct_journeys (view) → public.v_journeys → Frontend (useJourneys)
Freshness of the TV side = hourly cron, no dbt-build gate (all views down the chain);
the REST/Danish side still flows through stg_departures (view over raw, also live).
```
