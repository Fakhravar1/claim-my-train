...
**Current coverage (FULL SKÅNE NETWORK + SJ TRUNK TRIANGLE since 2026-06-23, collector v13).** `STATIONS` in `collect-train-announcements` lists **all ~80 Skåne stations** (crosswalked, verified 2026-06-16) **plus the SJ trunk triangle Stockholm–Malmö–Göteborg** (21 signatures added 2026-06-23: `Cst Flb Söö` Stockholm hub; `Nr Lp My Tns N Av` Södra stambanan; `K Hpbg Lå T Sk F Hrc A G` Västra stambanan; `Mdn Kb Vbc Fabp` Västkustbanan — all crosswalked, `name_match=true`). ~111 stations total. **Stockholm C (`Cst`) was reclassified from monitoring-only to a CLAIMABLE corridor** — removed from the `int_stop_events` TV-exclusion list (now EMPTY). Verified 2026-06-23: Malmö C↔Stockholm C journeys pair with operator `SJ` and `route_distance_km` ≈ 615 km (≥150 → EU band). Long-haul pairings fill in over a few hourly cron cycles (both endpoints must land in one ~2 h window). The pull `limit` is 20000 (a full pull is ~2.3k events; §15 gap #2's true pagination still only matters at the ~1000-station target). **Önnestad (`Önd`) remains polled-but-invisible** (null `ref_stations.rest_area_id` → dropped by the 740-regex join; resolve via ResRobot `location.name`). To add a station: look up `tv_signature` in `ref_stations`, append to `STATIONS`, redeploy (**keep `verify_jwt=false`**); add to the `int_stop_events` TV-exclusion list only if it should be monitoring-only.
