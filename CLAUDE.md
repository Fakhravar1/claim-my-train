...
**Next step (not built):** a cross-border journey mart pairing `int_stop_events` by `(train_number, service_date)`, origin/destination ordered by **scheduled time** (NOT `stop_sequence` — TV has none, and a 2-stop corridor doesn't need it). Destination delay → `is_claimable`.
