...
def handle_kalmar(sb, claim: dict) -> None:
    """Kalmar länstrafik web-form path (no BankID). Same review→authorize gate as HLT/SJ."""
    from submit_kalmar import submit_kalmar  # lazy: PDF path needs no Playwright

    cid = claim["id"]
    profile = load_profile(sb, claim)
    live = KLT_SUBMIT_LIVE and claim.get("status") == "kalmar_authorized"

    result = submit_kalmar(claim, profile, live=live)

    shot_path = None
    if result.get("screenshot"):
        shot_path = f'{claim["user_id"]}/{cid}-kalmar.png'
        sb.storage.from_(BUCKET).upload(
            shot_path, result["screenshot"], {"content-type": "image/png", "upsert": "true"}
        )
    ...
    if result.get("error"):
        ...
        print(f"  {cid}: kalmar -> {result['error']}")
    elif result.get("submitted"):
        sb.table("claims").update(
            {"status": "submitted", "external_reference": result.get("external_reference"),
             "submitted_at": datetime.now(timezone.utc).isoformat(),
        ...
        print(f"  {cid}: kalmar -> submitted (ref {result.get('external_reference')})")
    else:
        sb.table("claims").update(
            {"status": "awaiting_kalmar_authorization", "error_message": None, "pdf_path": shot_path}
        ).eq("id", cid).execute()
